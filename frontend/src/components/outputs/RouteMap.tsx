...
export function RouteMap({ eventLocation, language = 'ms' }: Props) {
  ...
  const t = language === 'en'
    ? {
        ...
        nearestPasar: 'Nearest wholesale market',
    ...
    : {
        title: 'Laluan Bekalan',
        from: 'Dari',
        to: 'Ke',
        distance: 'Jarak',
        driveTime: 'Masa Memandu',
        traffic: 'Kelewatan trafik',
        openMap: 'Buka di Bing Maps →',
        nearestPasar: 'Pasar Borong Terdekat',
  ...
  // Check if Azure Maps is enabled
  useEffect(() => {
  ...
  // Fetch route when location is available and maps enabled
  useEffect(() => {
